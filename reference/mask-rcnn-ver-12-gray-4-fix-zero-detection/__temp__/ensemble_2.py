...
def run_make_l2_data():
    ...
    out_dir = \
        '/root/share/project/kaggle/science2018/results/mask-se-resnext50-rcnn_2crop-mega-05d/ensemble'
    ...
    for i in range(len(ids)):
        ...
        average_of_augmentations = np.zeros((height, width),np.float32)
        instances=[]
        proposals=[]
        for t in range(num_ensemble):
            dir = ensemble_dirs[t]
            ...
            average_of_augmentations += instance.sum(0)

        # this is average of all augmentations
        average_of_augmentations = average_of_augmentations/num_ensemble
        average_of_augmentations = np.clip(average_of_augmentations,0,1)
        image_show_norm('average_of_augmentations', average_of_augmentations, max=1,resize=1)


        #loop trough one-bye-one
        display_low  = np.zeros((height, width), np.float)
        display_mid  = np.zeros((height, width), np.float)
        display_high = np.zeros((height, width), np.float)

        cluster_low  = []
        cluster_mid  = []
        cluster_high = []

        display = np.zeros((height, width,3), np.float)
        display[:,:,1] = average_of_augmentations
        num_augments = len(instances)
        for n in range(0, num_augments):
            proposal = proposals[n]
            instance = instances[n]
            ...
            for i in range(num):
                ...
                display[:,:,2] = m
                image_show_norm('display', display, resize=1, max=1, )
                image_show_norm('display_low', display_low, resize=1)
                image_show_norm('display_mid', display_mid, resize=1)
                image_show_norm('display_high', display_high, resize=1)
                image_show_norm('display_all', np.hstack([display_low,display_mid,display_high]), resize=1)
                cv2.waitKey(1)
