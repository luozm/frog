...
def run_npy_to_sumbit_csv():
    ...
    image_dir = '/root/share/project/kaggle/science2018/data/image/stage2_test/images'
    npy_dirs  = [
        '/root/share/project/kaggle/science2018/results/stage2_gray/ensemble1/masks',
        '/root/share/project/kaggle/science2018/results/stage2_color/ensemble1/masks',
    ]

    submit_dir  = \
        '/root/share/project/kaggle/science2018/results/stage2_color/submit1'
    csv_file = submit_dir  + '/submission-stage2-LB0.501.csv'
    ...
    for t,npy_file in enumerate(npy_files):
        ...
        #<debug> ------------------------------------
        print(t, name, all_num, num)  ##GT is 4152?
        ...
        if 1:
            ...
            #psd
            os.makedirs(submit_dir +'/overlays/%s'%name, exist_ok=True)
            cv2.imwrite(submit_dir +'/overlays/%s/%s.png'%(name,name),image)
            cv2.imwrite(submit_dir +'/overlays/%s/%s.mask.png'%(name,name),color_overlay)
            cv2.imwrite(submit_dir +'/overlays/%s/%s.contour.png'%(name,name),contour_overlay)
