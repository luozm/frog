"""
Transformation functions for both image and multi mask.
...
# -------------------------------------------------------
# General random transform functions
# -------------------------------------------------------
...
def random_transform2(image, mask, u, func, **kwargs):
    ...
    if random.random() < u:
        ...
        for k in kwargs:
            limit = kwargs[k]
            limits[k] = random.uniform(limit[0], limit[1])
...
# -------------------------------------------------------
# Specific illumination transforms
# -------------------------------------------------------
...
# -------------------------------------------------------
# Specific filters (blur, add noise)
# -------------------------------------------------------
...
#    random_dummy_transform(image, 0.5, dummy_transform,  limits1=[1,2], limits2=[4,5], limits3=[8,9]  )
    ...
    dataset = ScienceDataset('train1_all_670', img_folder='stage1_train', mask_folder='stage1_train', mode='train')
    for img, mask, _, index in dataset:
        img_trans, mask_trans = random_transform2(img, mask, u=0.5, func=do_elastic_transform2, grid=[16, 16], distort=[0.2, 0.2])
        plt.imshow(mask)
        plt.figure()
