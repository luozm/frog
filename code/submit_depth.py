...
from dataset.reader import ScienceDataset, multi_mask_to_color_overlay, mask_to_inner_contour, multi_mask_to_annotation
...
#        'test1_purple_12',
    ...
    for i, (inputs, images, indices) in enumerate(test_loader, 0):
        ...
        ids = read_list_from_file(SPLIT_DIR + 'test1_all_65', comment='#')
        ...
        for b in range(batch_size):
            ...
            mask_result = np.zeros(mask.shape, np.float32)
            depth = np.array(depth, np.int32)
            _, _, mask = multi_mask_to_annotation(mask)

            k = 0
            for i_ in range(len(mask)):
                depth[i_] = depth[i_]*mask[i_]
                depth[i_] = remove_small_objects(depth[i_], min_size=64)
                depth[i_] = remove_small_holes(depth[i_])
                labels = label(depth[i_])
                ids = np.unique(labels)[1:]
                for id_ in ids:
                    mask_temp = (labels == id_)
                    mask_temp = binary_dilation(mask_temp)
...
if __name__ == '__main__':
    ...
    run_submit(
        ...
        checkpoint='00084000_model.pth')
    # run_npy_to_sumbit_csv(
    #     image_dir=IMAGE_DIR + 'stage1_test/images',
    #     submit_dir=RESULTS_DIR + 'mask-rcnn-se-resnext50-depth-train500-train500-norm-01/submit/',
    #     csv_file='submission-senet-depth-norm-01.csv'
    # )
    # run_npy_to_sumbit_csv(
    #     image_dir=IMAGE_DIR + 'stage1_test/images',
