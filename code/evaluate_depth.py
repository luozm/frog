...
from skimage.morphology import remove_small_objects, remove_small_holes, label, binary_dilation
...
def revert(net, images):
    ...
    for b in range(batch_size):
        ...
        net.masks[b] = net.masks[b][:, :height, :width]
        net.depths[b] = net.depths[b][:height, :width]
        net.depths_resize[b] = net.depths_resize[b][:, :height, :width]
...
#            loss = net.loss(inputs, truth_boxes,  truth_labels, truth_instances)
        ...
        masks = net.masks
        depths = net.depths_resize
        depths_draw = net.depths
        ...
        for b in range(batch_size):
            ...
            mask_result = np.zeros(mask.shape[1:3], np.float32)
            depth = np.array(depth, np.int32)
            k = 1
            for i_ in range(len(depth)):
                '''if mask[i_].sum() < 8:
                    continue
                mask_temp = mask[i_].astype(np.bool)
                mask_result[mask_temp] = k
                k += 1'''
                depth[i_] = (depth[i_] > 1) * mask[i_]
                depth[i_] = depth[i_].astype(np.bool)
                depth[i_] = remove_small_objects(depth[i_], 32)
                depth[i_] = remove_small_holes(depth[i_], 256)
                labels = label(depth[i_])
                ids = np.unique(labels)[1:]
                if len(ids) <= 1:
                    mask[i_] = remove_small_objects(mask[i_], 16)
                    if mask[i_].sum() < 16:
                        continue
                    mask_temp = mask[i_].astype(np.bool)
                    mask_result[mask_temp] = k
                    ...
                    print(len(ids))
                    for id_ in ids:
                        mask_temp = (labels == id_)
                        if mask_temp.sum() < 16:
                            continue
                        mask_temp = binary_dilation(mask_temp, (np.ones((3,3))).astype(np.bool))
                        mask_result[mask_temp] = k
                        k += 1


            # depth = (depth > 0) * mask
            # depth = remove_small_holes(depth, 256)
            # labels = label(depth)
            # ids = np.unique(labels)[1:]
            # for id_ in ids:
            #     mask_temp = (labels == id_)
            #     mask_temp = binary_dilation(mask_temp, (np.ones((5, 5))).astype(np.bool))
            #     mask_result[mask_temp] = k
            #     k += 1

            # mask = instance_to_multi_mask(mask)
            # mask_result = filter_small(mask, 8)
            mask = mask_result
...
#            print('%d\t%s\t%0.5f  (%0.5f)'%(i,name,mask_average_precision, box_precision))
            ...
            # depth_draw = depths_draw[b]
            # depth_draw = image_show_color('predict depth', depth_draw, 1)
            # cv2.imwrite(out_dir + '/evaluate/%s.depth.png' % name, depth_draw)
...
#        'valid1_ids_gray2_43_nofolder',
        ...
        checkpoint=RESULTS_DIR + '/mask-rcnn-se-resnext50-depth-train500-train500-norm-01/checkpoint/00084000_model.pth')
