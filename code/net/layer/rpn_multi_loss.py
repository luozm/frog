...
def rpn_loss(logits, deltas, labels, label_weights, targets, target_weights, delta_sigma=3.0):
    ...
    batch_size, num_windows, _ = logits.size()
    labels = labels.long()
    ...
    # rpn_matchs = rpn_matchs.view(-1, 1)
    #
    # # find all logits corresponding to samples
    # # Positive and Negative anchors contribute to the loss,
    # # but neutral anchors (match value = 0) don't.
    # useful_indices = torch.ne(rpn_matchs, 0).expand(-1, 2)
    # logits_1 = torch.masked_select(logits, useful_indices)
    # logits_2 = logits_1.contiguous().view(-1, 2)
    ...
    # one hot encode
    deltas = deltas.view(-1, 2, 4)
    targets = targets.view(-1, 4)
    target_weights = target_weights.view(-1, 1)
    index = (labels != 0).nonzero()[:, 0]

    # # Positive anchors contribute to the loss, but negative and
    # # neutral anchors (match value of 0 or -1) don't.
    # index = (rpn_matchs == 1).nonzero()[:, 0]
    deltas = deltas[index]
