...
from dataset.reader import mask_to_inner_contour, multi_mask_to_contour_overlay
from dataset.transform_new import do_gamma, relabel_mask
...
def draw_multi_proposal_metric(image, proposal, truth_box, truth_label,
...
def draw_mask_metric(image, mask, truth_mask):
    ...
    overlay_truth_mask = multi_mask_to_contour_overlay(truth_mask, overlay_truth_mask, [0,255,255])
    ...
    #mask_score = cv2.cvtColor((mask_score/mask_score.m# yellow
    image_proposal = image.copy()  # pinkax()*255).astype(np.uint8),cv2.COLOR_GRAY2BGR)
...
def draw_multi_proposal_metric_depth(image, proposal, truth_box, truth_label, truth_depth,
        color0=[0,255,255], color1=[255,0,255], color2=[255,255,0],thickness=1):
    ...
    draw_shadow_text(truth_depth, 'depth', (5,15), 0.5, (255,255,255), 1)

    all = np.hstack([image_truth,image_proposal, image_hit, image_miss, image_fp, image_invalid, truth_depth])
    draw_shadow_text(all, '%0.2f prec@0.5' % precision, (5, H-15), 0.5, (255, 255, 255), 1)
    return all

def draw_mask_metric_depth(image, mask, depth, truth_mask, truth_depth):
    mask       = relabel_mask(mask)
    truth_mask = relabel_mask(truth_mask)
    norm_image = do_gamma(image, 2.5)
    ...
    draw_shadow_text(depth, 'depth', (5,15), 0.5,(255,255,255),1)

    all = np.hstack((overlay_truth_mask, overlay_mask, overlay_hit, overlay_miss, overlay_fp, image, depth))
    draw_shadow_text(all,'%0.2f prec@0.5' % precision_50, (5,H-45),0.5, (255,255,255), 1)
    draw_shadow_text(all,'%0.2f prec@0.7' % precision_70, (5,H-30),0.5, (255,255,255), 1)
    draw_shadow_text(all,'%0.2f prec'%average_precision,  (5,H-15),0.5, (255,255,0), 1)
    #image_show('all mask : image, truth, predict, error, metric',all,1)
